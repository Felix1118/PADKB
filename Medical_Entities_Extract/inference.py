...
MIMIC_REPORT_PATH='./mimic_report.csv'
...
def preprocess_reports():
    ...
    df=pd.read_csv(MIMIC_REPORT_PATH,sep=',')
    final_list=[]
    for idx, row in df.iterrows():

        temp_file = row['Report']
        sen = re.sub('(?<! )(?=[/,-,:,.,!?()])|(?<=[/,-,:,.,!?()])(?! )', r' ',temp_file).split()
        temp_dict = {}

        temp_dict["doc_key"] = row['Path']
...
def postprocess_reports():
    ...
    for file in tqdm(data,bar_format='{desc} [{n_fmt}/{total_fmt}]', desc='Processing'):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str, nargs='?', required=False,default = './model.tar.gz',
                        help='path to model checkpoint')
    
    parser.add_argument('--data_path', type=str, nargs='?', required=False,default = './data',
                        help='path to folder containing reports')
    
    parser.add_argument('--out_path', type=str, nargs='?', required=False,default = 'temp_dygie_output.json',
