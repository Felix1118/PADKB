...
class MultimodalPretrainingDataset(data.Dataset):
    ...
    def get_caption(self, captions,entities):
        series_sents=re.split(r'\.+', captions) 
        ...
        entity_tokens=self.tokenizer.tokenize(entities)
        entity_token_ids = self.tokenizer.convert_tokens_to_ids(entity_tokens)
        token_to_indices = defaultdict(list)
        for idx, token in enumerate(input_ids):
             if token in entity_token_ids:  
                token_to_indices[token].append(idx)
        all_indices = sorted({idx for indices in token_to_indices.values() for idx in indices})
        x_len = len([t for t in tokens["input_ids"][0] if t != 0])
        if(len(all_indices)<=1):
            all_indices=list(range(x_len))
        
        entity_index=torch.tensor(all_indices).unsqueeze(0)
        tokens['entity_index']=entity_index
    ...
    def __getitem__(self, index):
        ...
        caps, cap_len = self.get_caption(captions,entities)
...
def multimodal_collate_fn(batch):
    """sort sequence"""
    imgs, cap_len, ids, tokens, attention,entity_index = [], [], [], [], [], []
    path = []
    for b in batch:
        img, cap, cap_l, p = b
        imgs.append(img)
        ...
        entity_index.append(cap['entity_index'])
        path.append(p)
    ...
    entity_index=torch.stack(entity_index).squeeze()
    ...
    return_dict = {
        ...
        "entity_position_index": entity_index,
        "imgs": imgs,
        "cap_lens": sorted_cap_lens,
        'shape':entity_index.shape,
        "path": path
    }
    return return_dict
